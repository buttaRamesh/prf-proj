...
class Command(BaseCommand):
    ...
    def _build(self):
         cust_list = CustomerFactory.build_batch(self.size)
    ...
    def _create(self):
        cust_list = CustomerFactory.create_batch(self.size)
        self.stdout.write(self.style.SUCCESS(f'\nSaved {len(cust_list)} records'))

        if self.show:
            self._show_records(cust_list)
    def clean(self):
        CustomerFactory.delete_records()

    def _show_records(self,cust_list):
        sz = min(5,len(cust_list))
        for idx in range(sz):
            cust = cust_list[idx]
            self.stdout.write(self.style.HTTP_NOT_MODIFIED(f"   {cust.id} - {cust.gender} {cust.pan} {cust.bank_details}  "))
        print('')
